...
function commandFor(site, appPort) {
  ...
  if (manager === "pnpm") return { command: "pnpm.cmd", args: ["run", script, "--", "--port", String(appPort)] };
  if (manager === "yarn") return { command: "yarn.cmd", args: [script, "--port", String(appPort)] };
  if (manager === "bun") return { command: "bun.exe", args: ["run", script, "--", "--port", String(appPort)] };
...
function setupCommand(site) {
  ...
  if (site.manager === "pnpm") return { command: "pnpm.cmd", args: ["install"] };
  if (site.manager === "yarn") return { command: "yarn.cmd", args: ["install"] };
  if (site.manager === "bun") return { command: "bun.exe", args: ["install"] };
...
<main><div class="summary"><span id="count">Scanning references…</span><span>Setup installs dependencies only for the one reference you choose.</span></div><section class="grid" id="sites"></section></main>
...
function esc(s){return String(s).replace(/[&<>\"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[c]))} function render(){const q=search.value.toLowerCase();const list=sites.filter(s=>(s.name+' '+s.group+' '+s.packageName).toLowerCase().includes(q));count.textContent=list.length+' of '+sites.length+' cloned references';siteRoot.innerHTML=list.map(s=>'<article class="card"><div class="eyebrow">'+esc(s.group)+'</div><h2>'+esc(s.name)+'</h2><p>'+esc(s.path)+'</p><div class="tags"><span class="tag">'+esc(s.kind)+'</span><span class="tag">'+esc(s.manager)+'</span><span class="tag">'+(s.installed?'dependencies ready':'setup needed')+'</span></div>'+(s.script?'<button data-id="'+esc(s.id)+'">'+(s.installed?'LAUNCH '+esc(s.script).toUpperCase():'SET UP &amp; LAUNCH')+'</button>':'<button disabled>SOURCE ONLY</button>')+'</article>').join('')}
